...
class EcTrackJobTest extends TestCase {
    ...
    public function test_elevation_chart_images_generation() {
        ...
        $ecMediaDisk->shouldReceive('exists')
            ->with('ecmedia/ectrack/elevation_charts/23.svg')
            ->once()
            ->andReturn(false);
        ...
        $ecMediaDisk->shouldReceive('exists')
            ->with('ecmedia/ectrack/elevation_charts/23_old.svg')
            ->once()
            ->andReturn(false);

        $ecTrackService->generateElevationChartImages($ecTrack);
    }
}
